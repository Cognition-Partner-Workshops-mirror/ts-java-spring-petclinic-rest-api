...
class VetControllerTest {
    ...
    private final VetResponseDto carter = new VetResponseDto(1, "James", "Carter", List.of());
    private final VetResponseDto leary = new VetResponseDto(2, "Helen", "Leary",
        List.of(new SpecialtyResponseDto(1, "radiology")));

    @Test
    void listVets_returnsAll() throws Exception {
        when(vetService.findAll()).thenReturn(List.of(carter, leary));
    ...
    void listVets_filterByLastName() throws Exception {
        when(vetService.findByLastName("Carter")).thenReturn(List.of(carter));
    ...
    void listVets_filterBySpecialtyId() throws Exception {
        when(vetService.findBySpecialtyId(1)).thenReturn(List.of(leary));
    ...
    void listVets_filterBySpecialtyName() throws Exception {
        when(vetService.findBySpecialtyName("radiology")).thenReturn(List.of(leary));
    ...
    void listVets_blankLastName_returnsAll() throws Exception {
        when(vetService.findAll()).thenReturn(List.of(carter));
    ...
    @Test
    void listVets_blankSpecialtyName_returnsAll() throws Exception {
        when(vetService.findAll()).thenReturn(List.of(carter));

        mockMvc.perform(get("/api/vets").param("specialtyName", "  "))
            .andExpect(status().isOk());
    }

    @Test
    void getVet_existingId_returnsVet() throws Exception {
        when(vetService.findById(1)).thenReturn(carter);
    ...
    void addVet_validRequest_returns201() throws Exception {
        ...
        when(vetService.create(any())).thenReturn(leary);
    ...
    void updateVet_validRequest_returns200() throws Exception {
        ...
        when(vetService.update(eq(1), any())).thenReturn(updatedDto);
    ...
    void deleteVet_existingId_returns200() throws Exception {
        when(vetService.delete(1)).thenReturn(carter);
