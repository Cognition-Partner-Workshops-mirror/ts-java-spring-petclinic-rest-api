...
class SpecialtyControllerTest {
    ...
    void addSpecialty_duplicateName_returns400() throws Exception {
        ...
        mockMvc.perform(post("/api/specialties")
                ...
                .content(objectMapper.writeValueAsString(request)))
            .andExpect(status().isBadRequest())
            .andExpect(jsonPath("$.detail").value("Specialty with name 'radiology' already exists"));
    }
    ...
    void updateSpecialty_validRequest_returns200() throws Exception {
        ...
        when(specialtyService.update(eq(1), any(SpecialtyRequestDto.class)))
            .thenReturn(new SpecialtyResponseDto(1, "radiology-updated"));

        mockMvc.perform(put("/api/specialties/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.name", is("radiology-updated")));
    }
