...
class SpecialtyControllerTest {
    ...
    private static final String BASE_URL = "/petclinic/api/specialties";
    ...
    @Test
    void listSpecialties_withNameFilter() throws Exception {
        when(specialtyService.searchByName("rad"))
            .thenReturn(List.of(new SpecialtyResponseDto(1, "radiology")));

        mockMvc.perform(get(BASE_URL).param("name", "rad"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$[0].name").value("radiology"));
    }
    ...
    @Test
    void addSpecialty_validRequest() throws Exception {
    ...
    @Test
    void addSpecialty_invalidRequest_blankName() throws Exception {
    ...
    @Test
    void updateSpecialty_validRequest() throws Exception {
        ...
        when(specialtyService.updateSpecialty(eq(1), any(SpecialtyRequestDto.class)))
            .thenReturn(new SpecialtyResponseDto(1, "orthopedics"));
