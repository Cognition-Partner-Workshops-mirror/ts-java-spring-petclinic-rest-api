...
public class VetService {
    ...
    private Set<Specialty> resolveSpecialties(List<SpecialtyRequest> requests) {
        ...
        for (SpecialtyRequest req : requests) {
            List<Specialty> found = specialtyRepository.findByNameContainingIgnoreCase(req.name());
            if (!found.isEmpty()) {
                specialties.add(found.get(0));
            } else {
                Specialty s = new Specialty();
                s.setName(req.name());
                specialties.add(specialtyRepository.save(s));
            }
        }
        return specialties;
    }
