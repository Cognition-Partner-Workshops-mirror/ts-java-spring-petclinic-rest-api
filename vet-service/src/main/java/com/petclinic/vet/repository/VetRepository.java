...
public interface VetRepository extends JpaRepository<Vet, Integer> {
    ...
    @Query("SELECT DISTINCT v FROM Vet v WHERE LOWER(v.lastName) LIKE LOWER(CONCAT('%', :lastName, '%'))")
    List<Vet> searchByLastName(@Param("lastName") String lastName);

    @Query("SELECT DISTINCT v FROM Vet v WHERE LOWER(v.firstName) LIKE LOWER(CONCAT('%', :name, '%')) OR LOWER(v.lastName) LIKE LOWER(CONCAT('%', :name, '%'))")
