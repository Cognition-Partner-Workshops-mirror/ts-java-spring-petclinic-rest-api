...
public interface VetRepository extends JpaRepository<Vet, Integer> {
    ...
    @Query("SELECT v FROM Vet v WHERE LOWER(v.firstName) LIKE LOWER(CONCAT('%', :firstName, '%')) AND LOWER(v.lastName) LIKE LOWER(CONCAT('%', :lastName, '%'))")
    List<Vet> findByFirstNameAndLastNameContainingIgnoreCase(@Param("firstName") String firstName, @Param("lastName") String lastName);
}
