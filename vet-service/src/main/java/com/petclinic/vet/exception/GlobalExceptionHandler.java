...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleValidation(MethodArgumentNotValidException ex, HttpServletRequest request) {
        ...
        List<Map<String, String>> errors = ex.getBindingResult().getFieldErrors().stream()
            .map(fe -> {
                String rejectedValue = Objects.toString(fe.getRejectedValue(), "null");
                String defaultMessage = Objects.toString(fe.getDefaultMessage(), "Validation failed");
                String message = "Field '%s' %s (rejected value: %s)".formatted(
                    fe.getField(), defaultMessage, rejectedValue);
                return Map.of(
                    "message", message,
                    "field", fe.getField(),
